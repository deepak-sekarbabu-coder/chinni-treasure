...
* ~~**In-Memory Cache Not Shared Across Serverless Instances**~~
  ...
  * **RESOLVED (Aug 5, 2026):** Redis caching integrated (`src/lib/redis.ts` + `src/lib/redis-cache.ts`) with an in-memory fallback when `REDIS_URL` is unset. Catalog, category, latest, recent, order, tracking, and stats routes now cache through Redis with short TTLs.
  * **REFINED (Aug 14, 2026):** Cache namespaces are now owned by modules, not routes. `src/lib/catalogue-cache.ts` owns the catalogue caches (`products`, `categories`, `catlatest`, `catpage`, `recent`) plus `invalidateCatalogCaches()`; `src/lib/order-cache.ts` owns the order/track caches plus `invalidateOrderCache()` (which also clears stats); `src/lib/stats-cache.ts` owns the stats cache. `src/lib/cache-invalidate.ts` and `src/lib/products-cache.ts` were removed — invalidation clears exactly what the owning module owns (SCAN + DEL of the namespace in Redis plus the in-memory fallback), never a hardcoded namespace list. See the architecture diagrams.
